...
- All resume-related files live under `CV/` (source script + named outputs + public copies).
- Main output files (both written under `CV/`):
  - `CV/武奥迪_计算机_博士在读简历.html`
  - `CV/武奥迪_计算机_博士在读简历.pdf`
- Public website copies generated by the same script, served at `/CV/cv.html` and `/CV/cv.pdf`:
  - `CV/cv.html`
  - `CV/cv.pdf`
